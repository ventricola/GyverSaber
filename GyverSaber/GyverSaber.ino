...
// ---------------------------- НАСТРОЙКИ -------------------------------
#define NUM_LEDS 41         // число МИКРОСХЕМ на ленте
// На Arduino Nano с WS2812B проект нормально заводится
// на 40 светодиодах, уже на 50 при использовании SRAM на 85%
// возникают артефакты в работе TMRpcm и MPU6050 и подвисания.
// Можно засветить больше 100 светиков, но больше ничего работать не
// будет.
// Для нормальной работы нужно около 400 свободных байт в SRAM.
// При использовании SdFat вместо SD можно сэкономить почти
// 100 байт, это по идее больше 25 светодиодов (3 байта на каждый CRGB объект).
// toneAC нормально не работает в любом случае, нужно разбираться дальше.
...
#define R1 98000            // сопротивление резистора делителя    
#define R2 48000            // сопротивление резистора делителя
...
byte voltage_measure() {
  ...
  int volts = voltage / 2 * 100;    // у нас 2 банки. делим на сотку для расчёта
  if (volts > 387)
